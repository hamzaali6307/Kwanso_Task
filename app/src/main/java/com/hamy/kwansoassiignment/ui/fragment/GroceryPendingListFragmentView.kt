...
class GroceryPendingListFragmentView : Fragment(),
    CompleteSwipe.OnCompleteSwipeListener {
    ...
    private fun viewsUpdates() {
        GlobalScope.launch(Dispatchers.Main) {
            if (dbHelper.allPendingGrocery as ArrayList<Grocery> != null &&
                dbHelper.allPendingGrocery.isEmpty()
            ) {
                error_view.visibility = View.VISIBLE
            } else {
                error_view.visibility = View.GONE
                itemArray = dbHelper.allPendingGrocery as ArrayList<Grocery>
    ...
    override fun onSwiped(viewHolder: RecyclerView.ViewHolder?, position: Int) {
        dbHelper.updateGrocery(itemArray[position].id, Utility.ItemStatus.COMPLETETD.toString())
        viewsUpdates()
    }
}